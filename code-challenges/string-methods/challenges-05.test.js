...
const howMuchPencil = (str) => {
  let result = [];
  let pencilArray = str.split('');
  result.push(str);
  for (let i = 0; i < pencilArray.length; i++) {
      let slicedArray = pencilArray.slice(i+1);
      let newArray = slicedArray.join('');
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach((element) => {
    let sliceOne = element.slice(element.indexOf(' ') + 1);
...
const splitFoods = (recipe) => {
  ...
  recipe.ingredients.forEach((string) => {
    ...
    let arraySlice = toArray.slice(2);
    let arrayJoin = arraySlice.join(' ');
    result.push(arrayJoin);
  });
  return result;
};
...
const stepActions = (recipe) => {
  ...
  recipe.steps.forEach(string => {
    let toArray = string.split(' ');
    let arraySlice = toArray.slice(0,1);
    let arrayJoin = arraySlice.join(' ');
    result.push(arrayJoin);
  });
  return result;
...
const removeLastCharacters = (str, numberOfCharacters) => {
  ...
  let newArray = toArray.splice(toArray.length - numberOfCharacters);
